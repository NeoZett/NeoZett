# How about me: Neo Zetterberg

*Hello, I'm Neo. Look around and you might find something interesting.*

## Abstract 📄

I'm an innovative mind with extravagant thoughts, and an aspiring
developer. I am sixteen years old and started programming at around
ten. I love Computer Science and Psychology. In this document
I will explain more about myself.

On my free time, I usually read, write, program, spend time with
friends and family, or do something else.

## Table of Contents 📖

- [Abstract](#abstract-)
- [Table of Contents](#table-of-contents-)
- 1. [Prefatory](#1-prefatory-)
  - 1.1 [Introduction](#11-introduction-)
  - 2. [Background](#2-background-)
- 3. [Who am I?](#3-who-am-i-)
  - 3.1 [My Occupation](#31-my-occupation-)
  - 3.2 [My Education](#32-my-education-)

## 1. Prefatory ✨

### 1.1 Introduction 📌

Hello. My name is Neo, and I'm an aspiring developer. I love Computer Science and
Psychology. In this document I will explain more about myself.

On my free time, I usually read, write, program, spend time with
friends and family, or do something else.

I would really appreciate it if you read through this document thoroughly.

### 2. Background 📜

I am sixteen years old and started programming at around ten. Initially, it was
mostly web-development through a website called [koda.nu](https://koda.nu) and
it wasn't until much later that I started with HTML development.

After doing this for a some time, I had found myself a rabbit hole; and I
explored many different software utilities, such as [node.js](https://nodejs.org/en)
and batch files before I started with [Python](https://www.python.org/) development.

It was using Python that I first learned networking, parsing, and libraries, such
as the [PyGame](https://www.pygame.org/news) library and later SDL.

Finally, after already having learned a lot about the C-languages, I started
programming in C++. And that is where I am today.

My favorite languages are all of the C-languages and Assembly because I am
fascinated with Computer Science and how programming languages work.

## 3. Who am I? 👋

### 3.1 My Occupation 💼

I am currently studying at LBS (Ljud och Bild Skolan) here in Sweden.

### 3.2 My Education 🎓

Currently, I am studying Technology in secondary school.
